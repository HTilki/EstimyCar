---
title: "🚙 Estimy[Car]{style='color:#0F52BA;'} 🚙"
...
:::{.notes}
**HAS**: On s'est intéressé au marché de la voiture et on s'est posé la question de ce qu'on peut prédire sur les voitures. Quand on pense à ça, on pense forcément au prix, comment prédire le prix d'un véhivule?

**BUK** : La plupart des français utilisent la côte argus mais est-ce que vous vous êtes déjà demandé comment celle-ci fonctionne?
...
De plus cette cote est payante et manque de transparence sur la facon d'estimer la valeur d'un véhicule.

**BUK**:
Pour les autres estimateur gratuit en ligne, ils demandent souvent les données des utilisateurs en échange d'une estimation 

C'est pour ces raisons, que avons eu comme objectif de créer notre propre estimateur de prix de voiture.

Un estimateur accessible, gratuit et sans filtre.

**HAS**:
Pour ce faire, vu qu'on est que 2 jeunes étudiants et ne sommes pas entourés de professionnels, la seule option que nous avons est de recuperer des annonces de voitures d'occasion en ligne.

Avant de vous montrer comment on a recuperer tout ça on va directement passer au + croustillant qui est une démonstration de l'application que nous avons créée.
...
## Web Scraping 🕸️ {background-image="imgs/webscraping.jpeg" background-opacity="0.1"}

:::{.notes}
**BUK**: Alors maintenant qu'on a vu le produit final, on va le disséquer pour voir comment on en est arrivé là avec une approche technique!

**HAS**: C’est une technique d’extraction de données présentes sur le web.

**BUK**: Vous avez pu le voir peut être mais il y a des liens dans l'onglet acheteur qui renvoie vers le site "La Centrale".


**HAS**: Alors nous avons scrapé ce site et l'intégralité des 300 000 annonces qui étaient présentes sur le site au moment du scraping.
Avec plus de 150 marques, 900 modeles
...
- \+ de <span style="color:#4682B4">**900**</span> modèles

:::{.fragment}
Extraction en **3 parties**, présent dans le module `webscraping.py`:

  - Récupération des noms des marques et modèles
...
- Extraction de toutes les annonces disponibles sur le site à l'aide de la fonction `extract_toutes_annonces()`, elle permet :
  - d'itérer sur chaque marque et modèles de véhicule
...
:::{.notes}
**BUK**
Une fois toutes les pages récuperées, sous cette forme de liste **pointe la slide**, on récupère chacune des caractéristiques du véhicule pour chq annonce. 
...
## 🧼 Data cleaning 🧹{background-image="imgs/datacleaning3.jpeg" background-opacity="0.2"}
...
:::{.fragment}
Pour le nettoyage de la base de données, nous avons utilisé le package `Polars` 🐻‍❄️, qui possède de nombreux avantages tels que :  
:::
...
:::{.fragment}
C'est le module `datacleaning.py` qui contient toutes les fonctions essentielles pour résoudre certains problèmes.
:::
...
- Diviser la chaîne de caractère de `cylindre` en plusieurs variables avec `get_cylindre`: 
...
## Le gazoduc 🛢️{background-image="imgs/gazoduc1.jpeg" background-opacity="0.1"}
...
Une fois le dataframe nettoyé, nous avons désormais 16 variables au lieu de 10 auparavant, celui-ci exporté en format parquet car parquet a une Compression efficace et permet d'avoir des économies de stockage. 
:::
...
:::{.fragment}
Ce dataframe sera exporté en format `.parquet` qui permet de stocker des bases volumineuses à moindre coût. 
:::

## 📖 Machine learning 📖{background-image="imgs/ML1.jpeg" background-opacity="0.1"}
...
une fois la base de données nettoyé on va pouvoir commencer à l'exploiter et creer nos modèles de prediction. 

Pour des raisons d'efficacité, nous n'avons pas fait un seul modèle qui predit pour n'importe quel véhicule, car ceux ci était moins performant que d'avoir un modele dédié par marque de véhicule. c'est donc la méthode que nous avons choisi.

Une fois fixé, on ne peux pas choisir un model au hasard, nous avons donc effectué different test au préalable à l'aide du package sklearn qui possede.

Sklearn possede une documentation tres fourni et nous permet d'utiliser des Pipelines pour effectuer plusieurs opération d'un seul coup, ce qui facilite son utilisation. Cepednant ellenne fonctionne pas avec Dataframe polars.

Dans cette partie on va vous montrer le:
split des données que l'on a fait
l'étape de prétraitement
les modeles et grilles de parametres choisies
comment on exporte et recupere les meilleurs modeles
comment on effectue l'estimation du prix d'un vehicule

:::

Pour des raisons d'efficacité pour la prédiction, **chaque marque de véhicule aura son propre modèle de prédiction**.

Ces modèles de prédiction ne peuvent être choisis au hasard. Il faut effectuer des tests et choisir le modèle le plus adapté.
...
:::{.fragment}
L'utilisation de ce package à plusieurs avantages, tel que sa documentation et une utilisation avec des **pipelines** possibles. Cependant, il ne fonctionne pas avec des bases de données `Polars`🐻‍❄️.
:::

- split des données avec `split_data`
- préprocesseur avec `get_preprocessor`
...
- `predict_prix` pour charger le modèle et prédire
...
:::{.fragment}
**Données d'entrainement** : échantillon de <span style="color:#4682B4">**80%**</span> des observations de la marque

**Données de test** : les <span style="color:#4682B4">**20%**</span> restants.
:::
...
La plupart des modèles **ne supportent pas les variables qualitatives** et requièrent donc un prétraitement. 
...
:::{.fragment}
`StandardScaler` est utilisé pour **effectuer une mise à l'échelle des caractéristiques numériques**. Il standardise les données en soustrayant la moyenne et en divisant par l'écart-type, de sorte que chaque caractéristique ait une moyenne de **0** et une variance de **1**.
:::
...
:::{.notes}
**BUK**:
Nous avons testé différents modèles tel que les SVM, les Boosting, etc.. Mais les meilleurs modèles à chaque fois étaient : (ceux sur la slide)

Le nombre de modèle à creer étant de 40, le procédés est assez long. C'esst pour cela que pour optimiser notre temps nous avons décider de tester certaines grille de parametres en amont pour en determiner une finale, la plus réduite possible.

Nous allons donc rapidement vous présentez les résultats préliminaires qui nous on permis d'avoir une grille finale.

:::

Après quelques tests, nous avons décidé d'utiliser uniquement les modèles suivants : 
...
:::{.fragment}
La quantité de données étant importante, l'optimisation des paramètres peut-être très longue.⏳
::: 

:::{.fragment}
C'est pour cette raison que plusieurs grilles ont été testées en amont afin de déterminer une grille finale, voici les résultats intermédiaires.
:::
...
:::{.notes}
**BUK**:
Graph décroissant par rapport au nb d'annonces sur l'axe X
Au lieu de se lancer directement dans des validations croisées avec un grille de paramètres choisi aléatoirement, 

Nous avons testé des grilles de paramètres entre 0 et 15 voisins. Le maximum retenu était 10 voisins. Cette pour cette raison que nous avons retenu cette grille.
:::
...
## min_samples_leaf 🍃 {background-image="imgs/randomforest.jpeg" background-opacity="0.2"}
...
## n_estimators {background-image="imgs/RF2.jpeg" background-opacity="0.1"}
:::{.notes}
**HAS**:
Ici on avait testé plusieurs valeurs allant de 100 à 1000. 
On a décidé de garder uniquement 500 et 800
:::
...
## Le choix des meilleurs modèles 1️⃣
:::{.notes}
**HAS**: Après avoir optimisé les paramètres, on effectue une seconde validation croisée avec les nouvelles grilles. 
Voici donc à quoi ressemble les modèles utilisés et les grilles de parametres.
:::
...
...
    for model_name, model in models.items():
        GridSearchCV(model, param_grids[model_name], cv=5)
...
...
:::{.notes}
**HAS**:
Une fois l'optimisation des paramètres effectué via une seconde validation croisée. On garde le meilleur modèle parmis les trois, on l'entraine avec tout le dataset pour la marque en question, et on l'export en fichier joblib, ainsi que le preprocesseur pour réutilisation. 
:::

Une fois que les meilleurs modèles ont été trouvés, on les exporte :
...
:::{.fragment}
Pour ensuite les importer lorsque l'on voudra prédire les prix ! 💰
:::
...
:::{.fragment}
Notre objectif à partir de ce moment est de rendre notre travail accessible et agréable à utiliser. 😎
:::

## Streamlit 🚀{background-image="imgs/streamlit1.jpeg" background-opacity="0.1"}
:::{.notes}
**BUK**:
Streamlit est une bibliothèque Python qui simplifie considérablement le processus de création d'applications web interactif pour l'analyse de données et la visualisation.

Streamlit propre une palette de fonctionnalités. Avec des box, boutons, multiselect, etc.. les possibilités sont presques infinies.

Avantage de Streamlit, faciliter le déploiement sur le web. Avec le repos github, constamment connecté à l'application sur streamlit. Si on met a jour la branche main/master, ça met à jour l'application sur streamlit. 

Bien que l'on vous à déjà présenté l'application. Nous n'allons pas rentrer dans les détails de sa conception, car nous ne trouvons pas interessant le fait de vous montrer le code de la mis en page.

:::

**Streamlit** est une bibliothèque Python qui simplifie considérablement le processus de création d'applications web interactives pour l'analyse de données et la visualisation. 


Notre application s'organise en **trois parties** : 

- un accueil
- un onglet acheteur 🕵️‍♂️
- un onglet vendeur 💰

:::{.fragment}
Avantages : 

- Facilite le déploiement sur le web
- Mise à jour de l'application lorsque le GitHub est mis à jour
:::

## DuckDB 🦆{background-image="imgs/duckdb.jpeg" background-opacity="0.1"}

:::{.notes}
**HAS**:
Le seul aspect de notre application que nous allons vous présenter est l'utilisation de DuckDB qui nous a apporté beaucoup de chose. Premierement quesque c'est ?

l'API python de duck db est open source et permet d'intéragir rapidement avec des bases de données, 
en particulier sur de grands ensembles de données,
Il prend aussi en charge un sous-ensemble de SQL standard, ce qui le rend accessible aux utilisateurs familiers avec les requêtes SQL. 
Il prend également en charge des extensions pour l'analyse de données plus avancée comme par exemple la sortie en polars, pandas et bien d'autres.

Voici donc un exemple d'utilisation :
Ici l'avantage est de pouvoir intéragir avec notre base de données directement depuis le fichier. 
:::

**Avantages :** 

- Open source
- Rapide et capable de gérer de grands ensembles de données
- Prend en charge un sous-ensemble de SQL standard
- Sortie en `polars` 🐻‍❄️ DataFrame
...
## Code et qualité 🧪✅{background-image="imgs/pytest.jpeg" background-opacity="0.1"}
:::{.notes}
**BUK**:
Dans un soucis d'accessibilité, l'utilisation de poetry est obligatoire afin de permettre de reproduire rapidement l'environnement nécessaire pour faire tourner l'app en local par ex. 
Et permet aussi de connaitre les dépendances nécessaires pour faire tourner l'app qui peuvent être retrouvées dans un fichier toml.


Pout la visibilité et la cohérence du code, on a vérifier le typage des f° avec mypy et formater le codee à l'aide de `black`. 

**HAS**:
Nous avons donc testé nos différents modules à l'aide de pytest. 

Ces outils contribuent à la qualité, à la maintenabilité et la stabilité du code tout en améliorant l'efficacité du processus de développement.

:::

- Docstring pour chaque fonction et module
- Gestion des dépendances avec `poetry`.
- Vérification du typage avec `mypy`
- Formatage à l'aide de `black`
- Test des différents modules avec `pytest`



## What's Next ➡️🏎️{background-image="imgs/whats_next.jpg" background-opacity="0.1"}

:::{.notes}
**BUK**:
Comme sur la centrale, creer notre propre analyse de prix, pour dénicher les bonnes affaires. 

Si possible, essayer de dénicher ses bonnes affaires sur dautres site que la centrale (qui est réputé pour vendre des voitures à prix plutot élevé.)
Comme le bon coin ou des sites de ventes au enchere comme alcopa

Déployer une application sans bug, et optimisé pour le coté utilisateur. 
:::

**Nos objectifs pour la suite :** 

- Avoir une base de données actualisée et récente.
- Créer notre propre analyse de prix. 
- Essayer d'ajouter d'autres fonctionnalités (ex: nombre de portes).
- Essayer de scraper d'autres sites (ex : Le Bon Coin).
