...
def cv_result_into_df(modele: str, marques_array: np.ndarray): 
    for marque in marques_array:
        if marque[0] == marques_array[0,0]:
            cv_results = duckdb.sql(
                ...
                '{modele}' as modele,
                params
                FROM 'cv_results/cv_results_nestimators copy/{marque[0]}_{modele}_results.json'
    ...
    px.bar(cv_result_into_df("KNeighbors", marques_array), x='marque', y='mean_test_score', color = 'n_neighbors')
```
