...
Bien que l'on vous à déjà présenté l'application. L'objectif ici est de vous montrer le travail derrière
...
:::{.notes}
**HAS**:
l'API python de duck db est open source et permet d'intéragir rapidement avec des bases de données, en particulier sur de grands ensembles de données,Il prend aussi en charge un sous-ensemble de SQL standard, ce qui le rend accessible aux utilisateurs familiers avec les requêtes SQL. Il prend également en charge des extensions pour l'analyse de données plus avancée.
:::
...
## Poetry et Pytest 🧪✅{background-image="imgs/pytest.jpeg" background-opacity="0.1"}
:::{.notes}
**BUK**:
...
-> screen des tests passed de la console
