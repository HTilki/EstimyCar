...
def recup_marques_modeles() -> dict:
    ...
    for marque_div in div_voitures.find_all('div', class_='dropdown__itemsWrapper__brand-name'):
            if isinstance(marque_div, Tag):
                ...
                modeles_divs = cast(ResultSet[Tag], marque_div.find_next_siblings('div', class_='dropdown__itemsWrapper__model-name'))
                modeles = list()
            for modele_div in modeles_divs:
                if isinstance(modele_div, Tag):
                    labels = cast(ResultSet[Tag], modele_div.find_all('label', class_='Text_Text_text Controls_Controls_label Controls_Controls_right Controls_Controls_active Text_Text_label2'))
