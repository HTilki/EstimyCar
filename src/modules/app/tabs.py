        ...
        st.dataframe(
            ...
            column_config={
                ...
                "prix": st.column_config.NumberColumn(
                    "Prix 💰 (en euros)", format="%d €"
                ),
                "lien": st.column_config.LinkColumn("Lien 🔗"),
                "energie": st.column_config.TextColumn("Énergie ⚡️"),
                "boite": st.column_config.TextColumn("Boîte 🛠️"),
                "cylindre": st.column_config.NumberColumn("Cylindre 🛢️"),
                "puissance": st.column_config.NumberColumn("Puissance 🐎"),
                "kilometrage": st.column_config.NumberColumn("Kilométrage ⚙️"),
                "moteur": st.column_config.TextColumn("Moteur 💥"),
                "Véhicule": st.column_config.TextColumn("Véhicule 🚘"),
                "Position_marché": st.column_config.TextColumn("Position Marché ⚖️"),
            },
        )
    except:
...
def predict_button(
    ...
    if st.button("**Estimer la valeur du véhicule.**", key="predict_button"):
...
def get_prix_pred_displayed() -> None:
    ...
    if "prix_pred" in st.session_state:
        if st.session_state["prix_pred"] != None:
            ...
            st.metric(
                "**:red[Prix estimé :]** ", value="Erreur lors de l'estimation du prix."
            )
...
def get_prix_moy_displayed() -> None:
    ...
    if "prix_moy_pred" in st.session_state:
        if st.session_state["prix_moy_pred"] != 0:
            st.metric(
                "**:blue[Prix moyen :]** ",
                value=format_prix(st.session_state["prix_moy_pred"]),
...
def predict_km_fictif_button(
    ...
    """
    if st.button(
        "**Estimer la valeur de votre véhicule selon le kilométrage.**",
        ...
        try:
            ...
            st.error(
                "Erreur lors de la génération du graphique. Vérifiez que toutes les caractéristiques sont bien renseignées."
            )
