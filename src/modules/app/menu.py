...
def display_km(user_role: str) -> tuple[int, int] | int | None:
    ...
    if user_role == "Acheteur":
        ...
        km = st.sidebar.number_input(
            "Kilométrage", step=10000, min_value=0, max_value=1000000, key="km_input"
        )
        return int(km)
    return None
...
def marques_select(user_role) -> list | str | None:
    ...
    if user_role == "Acheteur":
        ...
        choix_marque = st.sidebar.selectbox(
            "Marque",
            options=marques,
...
def modeles_select(
    nom_marques_modeles: pl.DataFrame, choix_marque: list | str, user_role
) -> list | str | None:
    """
    ...
    if user_role == "Acheteur":
        if not choix_marque:  # Si aucune marque n'est sélectionnée
            ...
            modeles = (
                nom_marques_modeles.filter(pl.col("marque").is_in(choix_marque))
                .explode("modeles")
                .to_series(1)
            )
        choix_modeles = st.sidebar.multiselect(
            "Modèle",
            options=modeles,
        ...
        choix_modele = st.sidebar.selectbox(
            "Modèle",
            options=modeles,
...
def energie_select(user_role: str) -> list | str | None:
    ...
    if user_role == "Acheteur":
        energies = st.sidebar.multiselect(
            "Énergie",
...
def display_prix_selection() -> tuple:
    ...
    with prixmin:
        prix_min = st.number_input(
            "Prix min", step=1000, min_value=0, max_value=3000000
        )
    with prixmax:
        prix_max = st.number_input(
            "Prix max", step=1000, min_value=0, value=3000000, max_value=3000000
        )
...
def display_puissance() -> int:
    ...
    ch = st.sidebar.number_input(
        "Puissance", step=10, min_value=0, max_value=1500, key="puissance_input"
    )
    return int(ch)
...
def generation_select(choix_marque: str, choix_modele: str) -> str:
    ...
    choix_generation = st.sidebar.selectbox(
        "Génération",
        generations,
        index=None,
    ...
    if choix_generation == "Autre choix...":
        choix_generation_2 = st.sidebar.text_input(
            "Autre choix de génération",
...
def moteur_select(choix_marque: str, choix_modele: str) -> str:
    ...
    choix_moteur = st.sidebar.selectbox(
        "Moteur",
        moteurs,
        index=None,
    ...
    if choix_moteur == "Autre choix...":
        choix_moteur_2 = st.sidebar.text_input(
            "Autre choix de moteur",
...
def cylindre_select(choix_marque: str, choix_modele: str) -> str:
    ...
    choix_cylindre = st.sidebar.selectbox(
        "Cylindre",
        cylindres,
        index=None,
    ...
    if choix_cylindre == "Autre choix...":
        choix_cylindre_2 = st.sidebar.text_input(
            "Autre choix de cylindre", placeholder="Exemple: 4.0", key="cylindre_input"
        )
...
def finition_select(choix_marque: str, choix_modele: str) -> str:
    ...
    choix_finition = st.sidebar.selectbox(
        "Finition",
        finitions,
        index=None,
    ...
    if choix_finition == "Autre choix...":
        choix_finition_2 = st.sidebar.text_input(
            "Autre choix de finition",
            placeholder="Choisir une finition",
            key="finition_input",
        )
...
def batterie_select(energie: str) -> str:
    ...
    if energie == "Électrique":
        ...
        if choix_batterie == "Autre choix...":
            choix_batterie_2 = st.sidebar.text_input(
                "Autre choix de batterie",
